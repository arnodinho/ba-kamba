...
$business_click_default_page_color = array(
    'primary-color' => array(
        'value'     => '',
...
function business_click_layout_options_callback() {
    ...
    global $post , $business_click_default_layout_options, $business_click_single_post_image_align_options,$business_click_default_page_color;
    ...
    $business_click_single_page_color = $business_click_customizer_saved_values['business-color-page'];
    ...
    <table class="form-table page-meta-box">
        ...
        <!-- color setup -->
        <tr>
            <td colspan="4"><?php esc_html_e( 'Primary Color', 'business-click' ); ?></td>
        </tr>
        <tr>
            <td>
               
                <?php
                $business_click_page_color_meta = get_post_meta( $post->ID, 'business-color-page', true );
                if( false != $business_click_page_color_meta ){
                    $business_click_single_page_color = $business_click_page_color_meta;
                }
                foreach ($business_click_default_page_color as $field) {

                    if( !$business_click_page_color_meta ){
                        $business_click_page_color_meta = $field['value'];
                    }
                    ?>
                    <input id="<?php echo esc_attr( $field['value'] ); ?>" type="text" name="business-color-page" value="<?php echo esc_attr( $business_click_page_color_meta ); ?>"/> 
                    <label class="description" for="<?php echo esc_attr( $field['value'] ); ?>">
                        <?php echo esc_html( $field['label'] ); ?>
                    </label>
                <?php } // end foreach
...
function business_click_save_sidebar_layout( $post_id ) {
    ...
    /*Color options*/
    if(isset($_POST['business-color-page'])){
        $old = get_post_meta( $post_id, 'business-color-page', true );
        $new = sanitize_text_field( wp_unslash( $_POST['business-color-page'] ) );
        if ( $new && $new != $old ) {
            update_post_meta( $post_id, 'business-color-page', $new );
        } elseif ( '' == $new && $old ) {
            delete_post_meta( $post_id, 'business-color-page', $old );
        }
    }
}
