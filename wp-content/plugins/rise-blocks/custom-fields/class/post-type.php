...
class Post_Type{
	...
	public function register_post_type(){
	    ...
	    $defaults = array(
			...
			'add_new'            => esc_html__( 'Add New ', 'rise-blocks' ),
			'add_new_item'       => esc_html__( 'Add New ', 'rise-blocks' ) . esc_html( $name ),
			'edit_item'          => esc_html__( 'Edit ', 'rise-blocks' ) . esc_html( $name ),
			'new_item'           => esc_html__( 'New ', 'rise-blocks' ) . esc_html( $name ),
			'all_items'          => esc_html__( 'All ', 'rise-blocks' ) . esc_html( $plural ),
			'view_item'          => esc_html__( 'View ', 'rise-blocks' ) . esc_html( $name ),
			'search_items'       => esc_html__( 'Search ', 'rise-blocks' ) . esc_html( $plural ),
			'not_found'          => esc_html__( 'No ', 'rise-blocks') . esc_html( strtolower( $plural ) ) . esc_html__( ' found', 'rise-blocks' ),
			'not_found_in_trash' => esc_html__( 'No ', 'rise-blocks') . esc_html( strtolower( $plural ) ) . esc_html__( ' found in Trash', 'rise-blocks' ), 
	...
	public function add_taxonomy( $name, $args = array(), $labels = array() ){
		...
		if( ! empty( $name ) ){
			...
			$defaults = array(
	           ...
	           'search_items'      => esc_html__( 'Search ', 'rise-blocks' ) . esc_html( $plural ),
	           'all_items'         => esc_html__( 'All ', 'rise-blocks' ) . esc_html( $plural ),
	           'parent_item'       => esc_html__( 'Parent ', 'rise-blocks' ) . esc_html( $name ),
	           'parent_item_colon' => esc_html__( 'Parent ', 'rise-blocks' ) . esc_html( $name ),
	           'update_item'       => esc_html__( 'Update ', 'rise-blocks' ) . esc_html( $name ),
	           'new_item_name'     => esc_html__( 'New ', 'rise-blocks' ) . esc_html( $name ),
	...
	public function render_meta_box( $post, $box ){
		...
		if( is_array( $box[ 'args' ] ) ){
			wp_nonce_field( 'rise_blocks_custom_fields_meta_nonce', 'name_meta_nonce' );
			$class = 'single-mode';
		?>
			<div class="rise-blocks-custom-fields-wrapper clearfix">
				<div class="rise-blocks-custom-fields">
						<div class="rise-blocks-custom-fields-tab-wrapper">
							<?php if( count( $box[ 'args' ] ) > 1 ): $class = 'tab-mode'; ?>
								<ul class="rise-blocks-custom-fields-tab-navigation">
									<?php $count = 0; foreach( $box[ 'args' ] as $section => $fields ): ?>
										<li>
											<a href="#<?php echo esc_attr( $this->helper->uglify( $section ) ); ?>" class="rise-blocks-custom-fields-tab-menu <?php echo $count == 0 ? 'active' : ''; ?>">
							...
							<div class="<?php echo esc_attr( $class ); ?> rise-blocks-custom-fields-tab">
								<?php $count = 0; foreach( $box[ 'args' ] as $section => $fields ): ?>
									...
									<div id="<?php echo esc_attr( $this->helper->uglify( $section ) ); ?>" 
										class="rise-blocks-custom-fields-single-tab <?php echo esc_attr( $active ); ?>"
									>
										<?php
	...
	public function save( $post_id ){
		...
		if ( empty( $_POST ) || !isset(  $_POST[ 'name_meta_nonce' ] ) || ! wp_verify_nonce( $_POST[ 'name_meta_nonce' ], 'rise_blocks_custom_fields_meta_nonce' ) ) {
			return;
		}
